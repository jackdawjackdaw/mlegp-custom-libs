...
function(gp, ylab = "predicted output", graphStyle = 2, verbose = FALSE, no.plot = FALSE, parallel = FALSE, effects, gp.nums = 1:gp$numGPs, length.out = 21, param.values = NULL, xlab = NULL, PC.weights = FALSE, lower = NULL, upper = NULL, FANOVA = FALSE, ...) {
	...
	if (!is.null(gp$UD) && !PC.weights) {
		...
		return (plotMainEffectsUsingPCWeights(gp, ylab = ylab, graphStyle = graphStyle, 
			verbose = verbose, no.plot = no.plot, effects, param.values = param.values, xlab = xlab, lower = lower, upper = upper, parallel = parallel, ...) ) 
	}
	...
	for (i in 1:length(gp.nums)) {
		mainList[[i]] = list()
		mainList[[i]]$gp = gp[[gp.nums[i]]]
		mainList[[i]]$effect = effects
	}

	preds = NULL
	if (!parallel)  preds = lapply(mainList, calcMainEffect, index = index, lower = lower, upper = upper, verbose = verbose)
	if (parallel) {
		if (verbose) {
			cat("calculating main effects in parallel mode for GPs: "); cat(gp.nums); cat("...\n")
		} 
		preds = sfLapply(mainList, calcMainEffect, index = index, lower = lower, upper = upper, verbose = verbose)
		if (verbose) {
			cat("...done\n")
		}
	}
