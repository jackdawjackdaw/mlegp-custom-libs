...
function(int.effect, mu0 = NULL, lower = apply(gp$X,2,min), upper = apply(gp$X,2,max), maxpts = NULL, verbose = TRUE) {

  gp = int.effect$gp
  effectNums = int.effect$effects

  if (length(effectNums) != 2) {
          stop("effectNums must be of length 2 for ANOVAInteractionEffect")
          return (NULL)
  }

  if (verbose) {
	s = paste(gp$params[effectNums[1]], gp$params[effectNums[2]], sep = ":")
	s = paste("calculating FANOVA for ", s)
        cat(s)
	cat("\n")
  }
