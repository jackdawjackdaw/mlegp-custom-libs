`calcMainEffect` <-
function(main, index = NULL, length.out=21, lower = apply(gp$X,2,min), upper = apply(gp$X,2,max), verbose = FALSE) {

	gp = main$gp
	holdIndex = main$effect
 	if (verbose) { 
        	s = paste("calculating effect for", gp$params[holdIndex])
                cat(s)   
                cat("...\n")                
	}
