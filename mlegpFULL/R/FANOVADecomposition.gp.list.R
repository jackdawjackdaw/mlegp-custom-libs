...
function(gp, Interaction=TRUE, verbose = TRUE, outputs=1:gp$numGPs, lower = NULL, upper = NULL, maxpts = NULL, parallel = FALSE) {
	...
	for (i in outputs) {
		...
		nextAns = FANOVADecomposition(gp[[i]], Interaction = Interaction, verbose=verbose, lower = lower, upper = upper, maxpts = maxpts, parallel = parallel )
		if (is.null(ans)) {
			ans = nextAns
		}
		else {
